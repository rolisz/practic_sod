...
int main(int argc, char* argv[]) {
  ...
  if (recv(sServer,buffer,ntohl(len),0) <0) {
    printf("Eroare la primire mesaj");
  }
  int nrLinii;
  if (recv(sServer,(char*)&nrLinii,sizeof(int),0) < 0) {
    printf("Eroare la primirea nr linii");
  }
  nrLinii = ntohl(nrLinii);
  printf("Primit server %s %d\n",buffer,nrLinii);
